...
export const AboutCard = ({ title, description, image, background }: PropType) => {
  return (
    <div className={clsx('p-5 w-full lg:flex block justify-between bg-transparent border-[1px] border-solid border-black rounded-[15px] mt-10', {
      '!bg-[#6C9BFF]': background
    })}>
      <div className='flex w-full justify-between'>
        <div>
          <h3 className={clsx('text-lg lg:text-2xl font-semibold w-full', {
            'text-white': background
          })}>{title}</h3>
          <DescriptioneAbout description={description} background={background} className='hidden lg:block' />
        </div>
        <Image
      ...
      <DescriptioneAbout description={description} background={background} className='block lg:hidden' />
