...
export const Langmenu = () => {
  ...
  const [imgIDN, setImgIDN] = useState<boolean>(false)
  ...
  return (
    <Menu setActive={setActive}>
      <MenuItem setActive={setActive} active={active} image={imgIDN ? idnImg : enImg}>
        <div className="flex flex-col space-y-4 text-sm ">
          <div onClick={() => setImgIDN(false)}>
            <Links href={`/${path}`} className='flex items-center gap-2'>
              <Image src={enImg} width={25} height={25} alt="en" />
              <p>English</p>
            </Links>
          </div>
          <div onClick={() => setImgIDN(true)}>
